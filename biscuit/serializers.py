...
from biscuit.exceptions import UnsupportedFormat
from biscuit.utils import format_datetime, format_date, format_time, make_naive
try:
    import lxml
...
# Ugh & blah.
# So doing a regular dump is generally fine, since Biscuit doesn't usually
...
if yaml is not None:
    ...
    class BiscuitConstructor(SafeConstructor):
    ...
    BiscuitConstructor.add_constructor(u'tag:yaml.org,2002:python/unicode', BiscuitConstructor.construct_yaml_unicode_dammit)

    class BiscuitLoader(Reader, Scanner, Parser, Composer, BiscuitConstructor, Resolver):
        def __init__(self, stream):
            Reader.__init__(self, stream)
            Scanner.__init__(self)
            Parser.__init__(self)
            Composer.__init__(self)
            BiscuitConstructor.__init__(self)
...
class Serializer(object):
    ...
    def __init__(self, formats=None, content_types=None, datetime_formatting=None):
        ...
        self.datetime_formatting = getattr(settings, 'BISCUIT_DATETIME_FORMATTING', 'iso-8601')
    ...
    def format_datetime(self, data):
        ...
        or globally (via ``settings.BISCUIT_DATETIME_FORMATTING``).
    ...
    def from_yaml(self, content):
        ...
        return yaml.load(content, Loader=BiscuitLoader)
